...
try:
    ...
    from models import ChatterCNN, HybridChatterNet
    TORCH_OK = True
except ImportError:
    TORCH_OK = False
    ChatterCNN = HybridChatterNet = None  # type: ignore
...
# Architecture choices shown in the UI
...
class TrainingPage(BasePage):
    ...
    self._sources — ordered list of source dicts:
        { "name": str, "type": "xlsx"|"live",
          "series": {col: np.ndarray}, "length": int }
    ...
    Model architectures
    -------------------
    ChatterCNN       — pure 1-D CNN on OD waveform only
    HybridChatterNet — CNN fast-branch + MLP context-branch
                       Context features are the per-window mean of every
                       CONTEXT_TAGS column available in the source series.
                       Normalizer stats (ctx_mean, ctx_std) are stored on
                       the trained model instance for seamless inference.
    """
    ...
    def __init__(self, parent):
        ...
        self._arch_var  = tk.StringVar(value=_ARCH_CHOICES[1])  # default: Hybrid
    ...
    def _build_ui(self):
        ...
        ctrl_row1 = ttk.Frame(ctrl); ctrl_row1.pack(side="top", fill="x")
        ctrl_row2 = ttk.Frame(ctrl); ctrl_row2.pack(side="top", fill="x", pady=(2, 0))
        ...
        ttk.Label(mdl_f, text="Architecture:").grid(row=0, column=c, padx=(0, 2)); c += 1
        arch_cb = ttk.Combobox(mdl_f, textvariable=self._arch_var,
                                values=_ARCH_CHOICES, state="readonly", width=30)
        arch_cb.grid(row=0, column=c, padx=(0, 8)); c += 1
        ...
        ttk.Spinbox(mdl_f, textvariable=self._ws_size,
                    from_=240, to=96000, increment=240, width=7).grid(
    ...
    def _get_combined(self, col: str) -> np.ndarray:
        parts = []
        for s in self._sources:
            arr = np.asarray(s["series"].get(col,
                  np.full(s["length"], np.nan, dtype=np.float32)), dtype=np.float32)
            if len(arr) < s["length"]:
    ...
    def _rebuild_col_combo(self):
        ...
        if self._col_var.get() not in all_cols and all_cols:
            # Prefer the primary OD column
            for preferred in ("NDC_System_OD_Value", "Tag_value"):
    ...
    def _refresh_src_list(self):
        ...
        for i, s in enumerate(self._sources):
            ...
            self._src_lb.itemconfig(i, background=_SRC_BANDS[i % len(_SRC_BANDS)])
    ...
    def _update_info(self):
        ...
        self._info_var.set(f"{n} source(s)  •  {t:,} total samples  •  {lw} labeled window(s)")
    ...
    def _redraw_plot(self):
        ...
        self._ax.plot(xs, combined, linewidth=0.7, color="#3B82F6", alpha=0.85, rasterized=True)
        ...
        for i, s in enumerate(self._sources):
            ...
            label = f"{s['name']}\n{'✓' if has else '✗ (no data)'}"
            self._ax.text(
                x0 + s["length"] / 2, 1.0, label,
                transform=self._ax.get_xaxis_transform(),
                ha="center", va="bottom", fontsize=7, color="#1E293B",
                bbox=dict(boxstyle="round,pad=0.25", fc=band_c, ec="#CBD5E1",
        ...
        self._ax.set_title(
            f"{col}  –  {len(self._sources)} source(s), {self._total_length():,} samples  |  "
    ...
    def _on_drag(self, event):
        ...
        if event.inaxes is not self._ax: return
    ...
    def _clear_all_windows(self):
        if self._windows and not messagebox.askyesno("Clear All", "Remove all labeled windows?"):
            return
        self._windows.clear()
        self._refresh_win_list(); self._redraw_plot(); self._check_train_ready()
    ...
    def _read_sheet_values(xl: pd.ExcelFile, sheet: str):
        try:
            df = xl.parse(sheet, usecols=["t_stamp", "Tag_value"],
                          dtype={"Tag_value": np.float32}, parse_dates=False)
    ...
    def _connect_ws(self):
        if ws_connect is None:
            messagebox.showerror("WebSocket", "websockets package not installed.")
            return
        ...
        if live_src is None:
            live_src = {"name": "Live (WS)", "type": "live", "series": {}, "length": 0}
    ...
    def _poll_live(self):
        batch = []
        while True:
            try:   batch.append(self._live_q.get_nowait())
            ...
            if live_src is not None:
                ...
                for msg_dict in batch:
                    for key, val in msg_dict.items():
                        live_src["series"].setdefault(key, []).append(val)
                if live_src["series"]:
    ...
    def _import_model(self):
        if not TORCH_OK:
            messagebox.showerror("PyTorch", "PyTorch not installed.")
            return
        path = filedialog.askopenfilename(
            title="Import Model",
            filetypes=[("PyTorch model", "*.pt *.pth"), ("Pickle", "*.pkl"),
                       ("All files", "*.*")])
        if not path: return
        try:
    ...
    def _start_training(self):
        ...
        col          = self._col_var.get()
        ws           = self._ws_size.get()
        use_hybrid   = "Hybrid" in self._arch_var.get()
        LABEL_MAP    = {"no_chatter": 0, "chatter": 1}

        X_list, y_list, ctx_list = [], [], []
        skipped_cross = skipped_no_col = 0
        ...
        for w in self._windows:
            ...
            if src is None:
                skipped_cross += 1; continue
            if col not in src["series"]:
                skipped_no_col += 1; continue
            ...
            if use_hybrid:
                ...
                for tag in CONTEXT_TAGS:
                    if tag in src["series"]:
                        raw = np.asarray(src["series"][tag][local_i0:local_i1],
                                         dtype=np.float32)
                        valid = raw[~np.isnan(raw)]
                        ctx_vec.append(float(valid.mean()) if len(valid) > 0 else 0.0)
                    else:
                        ctx_vec.append(0.0)
        ...
        if not X_list:
            ...
            if skipped_cross:  parts.append(f"  • {skipped_cross} cross-boundary window(s)")
            if skipped_no_col: parts.append(f"  • {skipped_no_col} missing-variable window(s)")
        ...
        # Compute context normalizer and normalised tensor
        X_ctx     = None
        ctx_mean  = None
        ctx_std   = None
        if use_hybrid and ctx_list:
            ctx_arr  = np.array(ctx_list, dtype=np.float32)  # (N, N_CONTEXT)
            ctx_mean = ctx_arr.mean(axis=0)
            ctx_std  = ctx_arr.std(axis=0) + 1e-8
            ctx_norm = (ctx_arr - ctx_mean) / ctx_std
            X_ctx    = torch.tensor(ctx_norm, dtype=torch.float32)
        ...
        n_ctx_ok = len(ctx_list) if use_hybrid else 0
        status(f"Training {'HybridChatterNet' if use_hybrid else 'ChatterCNN'} "
               f"on {len(X_list)} windows{skip_msg} "
               f"({'context from ' + str(N_CONTEXT) + ' tags' if use_hybrid else 'OD only'})…")

        threading.Thread(
            target=self._train_thread,
            args=(X, y, self._epochs.get(), lr, use_hybrid, X_ctx, ctx_mean, ctx_std),
            daemon=True,
        ).start()

    def _train_thread(self, X, y, epochs, lr,
                      use_hybrid, X_ctx, ctx_mean, ctx_std):
        try:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

            # Instantiate the correct architecture
            if use_hybrid:
                model = HybridChatterNet().to(device)
            else:
                model = ChatterCNN().to(device)

            # Warm-start from previously imported model if architectures match
            ...
            # Build dataset
            if use_hybrid and X_ctx is not None:
                ds     = TensorDataset(X.to(device), X_ctx.to(device), y.to(device))
            else:
            ...
            # Store normalizer on model so inference can apply the same transform
            trained = model.cpu()
            if use_hybrid and ctx_mean is not None:
                trained.ctx_mean = ctx_mean
                trained.ctx_std  = ctx_std
    ...
    def _training_done(self):
        ...
        self._save_btn.config(state="normal")
        arch = "HybridChatterNet" if "Hybrid" in self._arch_var.get() else "ChatterCNN"
        status(f"{arch} training complete")
